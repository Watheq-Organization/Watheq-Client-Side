...
export const OutstandingDebtsReportView: FC = () => {
  ...
  const renderKPIs = () => {
    if (!report?.summary || report.summary.length === 0) {
      return (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xs p-6 text-center text-slate-500 dark:text-slate-400 font-cairo">
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {report.summary.map((s) => (
          <div
            key={s.currencyCode}
            className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xs p-5 hover:shadow-md transition-shadow relative overflow-hidden"
          >
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-bold font-cairo text-[#051838] dark:text-white">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold font-cairo text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            الديون المستحقة
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            onClick={fetchReport}
            disabled={isLoading}
            className="inline-flex items-center gap-1.5 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800/50 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-emerald-700 rounded-xl text-sm font-medium hover:bg-emerald-50 transition-colors"
          >
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xs overflow-hidden">
        {/* Filters Bar */}
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-3 pr-9 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#051838] dark:focus:ring-blue-500 w-full sm:w-64"
              />
            </div>
            ...
            <select
              ...
              className="px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#051838] dark:focus:ring-blue-500"
            >
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">الديون المتأخرة فقط</span>
            </label>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800/50 transition-colors"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400 font-medium">
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-400">
                    <RotateCw className="w-6 h-6 animate-spin mx-auto text-[#051838] dark:text-white mb-2" />
                    جاري تحميل البيانات...
                  </td>
                </tr>
              ) : report?.details?.items && report.details.items.length > 0 ? (
                report.details.items.map((item) => (
                  <tr key={item.debtId} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-bold text-[#051838] dark:text-white">
                      #{item.debtId}
                    </td>
                    <td className="px-6 py-4">
                      <p className="font-semibold text-slate-800 dark:text-slate-200">{item.customerName}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{item.customerPhone}</p>
                    </td>
                    <td className="px-6 py-4 font-cairo font-semibold text-slate-600 dark:text-slate-400">
                      {item.amount.toLocaleString('en-US')} <span className="text-xs">{item.currencyCode}</span>
                    </td>
                    <td className="px-6 py-4 font-cairo font-bold text-[#051838] dark:text-white">
                      {item.remainingAmount.toLocaleString('en-US')} <span className="text-xs">{item.currencyCode}</span>
                    </td>
                    <td className="px-6 py-4 text-center font-cairo text-slate-600 dark:text-slate-400">
                    ...
                    <td className="px-6 py-4 text-center">
                      {item.daysOverdue > 0 || item.status === 'Overdue' ? (
                        ...
                        <span className="inline-block px-2.5 py-1 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-md text-xs font-bold border border-slate-200 dark:border-slate-700">
                          غير مسدد
                        </span>
                      )}
                    </td>
                ...
                <tr>
                  <td colSpan={7} className="py-16 text-center text-slate-400">
                    ...
                    <p className="font-bold text-slate-600 dark:text-slate-400">لا توجد ديون مستحقة</p>
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-800/50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
