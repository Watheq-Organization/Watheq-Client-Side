...
export const HelpCenterPage: FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-[#f8fafc] dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-cairo antialiased selection:bg-emerald-100 selection:text-emerald-900">
      <Navbar />
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-10 py-10 lg:py-16 space-y-16">
        ...
        <section className="text-center max-w-3xl mx-auto space-y-6 pt-4">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0c2444] dark:text-blue-400 font-cairo tracking-tight">
          ...
          <div className="relative max-w-2xl mx-auto mt-6">
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-2 sm:p-2.5 border border-slate-200 dark:border-slate-700 shadow-lg shadow-slate-200/50 flex items-center gap-2 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10 transition-all">
              <div className="flex-grow flex items-center gap-3 px-3">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-none text-slate-800 dark:text-slate-200 text-sm sm:text-base focus:outline-none placeholder:text-slate-400"
                />
                {searchQuery && (
                  <button
                    onClick={() => setSearchQuery('')}
                    className="p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-600 dark:bg-slate-700 text-slate-400 transition"
                  >
                    <X className="w-4 h-4" />
                  </button>
                )}
        ...
        <section className="space-y-8">
          <div className="text-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#0c2444] dark:text-blue-400 font-cairo">
              تصفح حسب الفئة
            </h2>
          </div>
            ...
            {filteredCategories.map((cat) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-slate-800 rounded-2xl p-6 border transition-all duration-200 cursor-pointer flex flex-col justify-between text-right group ${
                    isSelected
                      ? 'border-emerald-500 ring-2 ring-emerald-500/20 shadow-md'
                      : 'border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-slate-200 dark:border-slate-700 hover:-translate-y-0.5'
                  }`}
                >
                  <div className="space-y-4">
                    <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-300 group-hover:bg-emerald-50 group-hover:text-emerald-600 group-hover:border-emerald-100 transition-colors">
                      <IconComp className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="font-bold text-[#0c2444] dark:text-blue-400 text-lg font-cairo group-hover:text-emerald-700 transition-colors mb-2">
                        {cat.title}
                      </h3>
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-[#0c2444] dark:text-blue-400 font-cairo">
              المقالات الشائعة
            </h2>
            <button
              onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {filteredArticles.map((article) => (
              <div
                ...
                className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-slate-200 dark:border-slate-700 transition-all duration-200 cursor-pointer flex flex-col justify-between group"
              >
                <div className="space-y-3 text-right">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base group-hover:text-emerald-600 transition-colors leading-snug">
                      {article.title}
                    </h3>
          ...
          <div className="max-w-2xl mx-auto space-y-5 relative z-10">
            ...
            <div className="pt-3">
              <button
                ...
                className="bg-white dark:bg-slate-800 text-[#0c2444] dark:text-blue-400 hover:bg-slate-100 dark:hover:bg-slate-600 dark:bg-slate-700 px-8 py-3.5 rounded-xl font-bold text-sm inline-flex items-center gap-2.5 shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105 active:scale-95 cursor-pointer"
              >
      ...
      {selectedArticle && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div
            className="bg-white dark:bg-slate-800 rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-slate-100 dark:border-slate-700 text-right space-y-5 relative animate-fade-in"
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 dark:border-slate-700 pb-4">
              <div>
                ...
                <h3 className="text-xl font-bold text-[#0c2444] dark:text-blue-400 font-cairo">
                  {selectedArticle.title}
                </h3>
              </div>
              <button
                onClick={() => setSelectedArticle(null)}
                className="p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 dark:bg-slate-700 text-slate-400 hover:text-slate-600 dark:text-slate-400 transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="text-slate-600 dark:text-slate-400 text-sm sm:text-base leading-relaxed whitespace-pre-line py-2">
